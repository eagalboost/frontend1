...
const VideoMarketing = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Video Marketing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Video Marketing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Video Marketing services found.</p>
        )}
      </div>
    </section>
  );
