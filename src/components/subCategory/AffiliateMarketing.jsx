...
const AffiliateMarketing = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Affiliate Marketing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Affiliate Marketing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Affiliate Marketing services found.</p>
        )}
      </div>
    </section>
  );
