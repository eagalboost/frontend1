...
const BrandStyleGuides = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Brand Style Guides`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Brand Style Guides Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Brand Style Guides services found.</p>
        )}
      </div>
    </section>
  );
...
export default BrandStyleGuides;
