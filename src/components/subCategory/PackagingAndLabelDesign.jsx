...
const PackagingAndLabelDesign = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Packaging And Label Design`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Packaging and Label Design Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Packaging and Label Design services found.</p>
        )}
      </div>
    </section>
  );
...
export default PackagingAndLabelDesign;
