...
const EcommerceMarketing = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Ecommerce Marketing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Ecommerce Marketing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Ecommerce Marketing services found.</p>
        )}
      </div>
    </section>
  );
