...
const BlogWriting = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Blog Writing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Blog Writing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Blog Writing services found.</p>
        )}
      </div>
    </section>
  );
