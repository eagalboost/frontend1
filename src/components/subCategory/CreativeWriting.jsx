...
const CreativeWriting = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Creative Writing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Creative Writing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Creative Writing services found.</p>
        )}
      </div>
    </section>
  );
