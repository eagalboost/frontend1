...
const EcommerceSeo = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Ecommerce SEO`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Ecommerce SEO Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Ecommerce SEO services found.</p>
        )}
      </div>
    </section>
  );
