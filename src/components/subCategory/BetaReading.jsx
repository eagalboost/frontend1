...
const BetaReading = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Beta Reading`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Beta Reading Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Beta Reading services found.</p>
        )}
      </div>
    </section>
  );
