...
const MotionTracking = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Motion Tracking`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Motion Tracking Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Motion Tracking services found.</p>
        )}
      </div>
    </section>
  );
