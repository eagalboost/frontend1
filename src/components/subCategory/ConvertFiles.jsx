...
const ConvertFiles = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Convert Files`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Convert Files Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Convert Files services found.</p>
        )}
      </div>
    </section>
  );
