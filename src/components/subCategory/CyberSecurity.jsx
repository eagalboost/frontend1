...
const CyberSecurity = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Cyber Security`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Cyber Security Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Cyber Security services found.</p>
        )}
      </div>
    </section>
  );
