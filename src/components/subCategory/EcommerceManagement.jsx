...
const EcommerceManagement = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Ecommerce Management`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Ecommerce Management Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Ecommerce Management services found.</p>
        )}
      </div>
    </section>
  );
