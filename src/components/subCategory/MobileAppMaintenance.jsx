...
const MobileAppMaintenance = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Mobile App Maintenance`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Mobile App Maintenance Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No mobile app maintenance services found.</p>
        )}
      </div>
    </section>
  );
...
export default MobileAppMaintenance;
