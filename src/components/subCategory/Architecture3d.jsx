...
const Architecture3d = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=3D Architecture`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Architecture 3D Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Architecture 3D services found.</p>
        )}
      </div>
    </section>
  );
