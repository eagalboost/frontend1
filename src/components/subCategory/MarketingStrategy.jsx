...
const MarketingStrategy = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Marketing Strategy`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Marketing Strategy Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Marketing Strategy services found.</p>
        )}
      </div>
    </section>
  );
