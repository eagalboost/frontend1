...
const BookCovers = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Book Covers`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Book Covers Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Book Covers services found.</p>
        )}
      </div>
    </section>
  );
