...
const ThumbnailDesign = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Thumbnail Design`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Thumbnail Design Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Thumbnail Design services found.</p>
        )}
      </div>
    </section>
  );
