...
const CloudComputing = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Cloud Computing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Cloud Computing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Cloud Computing services found.</p>
        )}
      </div>
    </section>
  );
