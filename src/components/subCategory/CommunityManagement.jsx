...
const CommunityManagement = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Community Management`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Community Management Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Community Management services found.</p>
        )}
      </div>
    </section>
  );
