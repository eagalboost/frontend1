...
const SpeachWriting = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Speach Writing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Speech Writing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Speech Writing services found.</p>
        )}
      </div>
    </section>
  );
