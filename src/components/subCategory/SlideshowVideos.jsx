...
const SlideshowVideos = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Slideshow Videos`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Slideshow Videos Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Slideshow Videos services found.</p>
        )}
      </div>
    </section>
  );
