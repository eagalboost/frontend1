...
const PresentationDesign = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Presentation Design`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Presentation Design Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Presentation Design services found.</p>
        )}
      </div>
    </section>
  );
