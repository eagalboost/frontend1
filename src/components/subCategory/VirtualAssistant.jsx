...
const VirtualAssistant = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Virtual Assistant`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Virtual Assistant Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Virtual Assistant services found.</p>
        )}
      </div>
    </section>
  );
