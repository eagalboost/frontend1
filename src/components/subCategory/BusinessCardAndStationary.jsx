...
const BusinessCardAndStationary = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Business Card and Stationary`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Business Card & Stationary Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Business Card & Stationary services found.</p>
        )}
      </div>
    </section>
  );
...
export default BusinessCardAndStationary;
