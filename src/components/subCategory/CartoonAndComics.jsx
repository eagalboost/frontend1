...
const CartoonAndComics = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Cartoon And Comics`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Cartoon And Comics Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Cartoon And Comics services found.</p>
        )}
      </div>
    </section>
  );
...
export default CartoonAndComics;
