...
const IntroAndOutroVideo = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Intro and Outro Videos`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Intro and Outro Video Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Intro and Outro Video services found.</p>
        )}
      </div>
    </section>
  );
...
export default IntroAndOutroVideo;
