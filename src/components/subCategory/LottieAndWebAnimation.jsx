...
const LottieAndWebAnimation = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Lottie and Web Animation`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Lottie and Web Animation Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Lottie and Web Animation services found.</p>
        )}
      </div>
    </section>
  );
...
export default LottieAndWebAnimation;
