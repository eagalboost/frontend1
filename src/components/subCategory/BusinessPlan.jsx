...
const BusinessPlan = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Business Plan`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Business Plan Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Business Plan services found.</p>
        )}
      </div>
    </section>
  );
