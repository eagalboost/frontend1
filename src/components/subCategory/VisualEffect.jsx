...
const VisualEffect = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Visual Effect`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Visual Effect Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Visual Effect services found.</p>
        )}
      </div>
    </section>
  );
