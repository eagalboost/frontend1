...
const WhiteBoardAnimation = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=WhiteBoard Animation`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">WhiteBoard Animation Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No WhiteBoard Animation services found.</p>
        )}
      </div>
    </section>
  );
