...
const ExplainerVideos = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Explainer Videos`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Explainer Videos Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Explainer Videos services found.</p>
        )}
      </div>
    </section>
  );
