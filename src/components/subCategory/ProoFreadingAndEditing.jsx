...
const ProofreadingAndEditing = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Proofreading and Editing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Proofreading and Editing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Proofreading and Editing services found.</p>
        )}
      </div>
    </section>
  );
...
export default ProofreadingAndEditing;
