...
const ExplainerVideoProduction = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Explainer Video Production`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Explainer Video Production Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Explainer Video Production services found.</p>
        )}
      </div>
    </section>
  );
...
export default ExplainerVideoProduction;
