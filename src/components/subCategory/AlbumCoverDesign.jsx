...
const AlbumCoverDesign = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Album Cover Design`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Album Cover Design Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Album Cover Design services found.</p>
        )}
      </div>
    </section>
  );
...
export default AlbumCoverDesign;
