...
const WebSiteMaintenance = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Website Maintenance`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Website Maintenance Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No website maintenance services found.</p>
        )}
      </div>
    </section>
  );
