...
const MarketingResearch = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Marketing Research`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Marketing Research Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Marketing Research services found.</p>
        )}
      </div>
    </section>
  );
