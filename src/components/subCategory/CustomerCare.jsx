...
const CustomerCare = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Customer Care`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Customer Care Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Customer Care services found.</p>
        )}
      </div>
    </section>
  );
