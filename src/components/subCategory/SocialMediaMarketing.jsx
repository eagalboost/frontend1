...
const SocialMediaMarketing = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Social Media Marketing`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Social Media Marketing Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Social Media Marketing services found.</p>
        )}
      </div>
    </section>
  );
