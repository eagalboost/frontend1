...
const CallCenterAndCalling = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Call Center and Calling`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Call Center and Calling Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Call Center and Calling services found.</p>
        )}
      </div>
    </section>
  );
...
export default CallCenterAndCalling;
