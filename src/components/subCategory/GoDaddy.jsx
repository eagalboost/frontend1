...
const GoDaddy = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=GoDaddy`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">GoDaddy Development Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No GoDaddy development services found.</p>
        )}
      </div>
    </section>
  );
