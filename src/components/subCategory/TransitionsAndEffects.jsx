...
const TransitionsAndEffects = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Transitions and Effects`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Transitions and Effects Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Transitions and Effects services found.</p>
        )}
      </div>
    </section>
  );
...
export default TransitionsAndEffects;
