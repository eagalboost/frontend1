...
const PaidSocialMedia = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Paid Social Media`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Paid Social Media Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Paid Social Media services found.</p>
        )}
      </div>
    </section>
  );
