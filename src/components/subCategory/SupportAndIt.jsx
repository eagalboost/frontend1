...
const SupportAndIt = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Support and IT`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Support and IT Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Support and IT services found.</p>
        )}
      </div>
    </section>
  );
