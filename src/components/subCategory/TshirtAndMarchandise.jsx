...
const TshirtAndMerchandise = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Tshirt And Merchandise`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Tshirt And Merchandise Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Tshirt And Merchandise services found.</p>
        )}
      </div>
    </section>
  );
...
export default TshirtAndMerchandise;
