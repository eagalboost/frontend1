...
const BookFormatting = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Book Formatting`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Book Formatting Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Book Formatting services found.</p>
        )}
      </div>
    </section>
  );
