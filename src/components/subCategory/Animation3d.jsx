...
const Animation3d = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=3D Animation`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Animation 3D Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Animation 3D services found.</p>
        )}
      </div>
    </section>
  );
