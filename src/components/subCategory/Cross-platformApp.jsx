...
const CrossPlatformApp = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Cross-platform Apps`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Cross-platform App Development Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No cross-platform app development services found.</p>
        )}
      </div>
    </section>
  );
