...
const DisplayAdvertising = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Display Advertising`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Display Advertising Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Display Advertising services found.</p>
        )}
      </div>
    </section>
  );
