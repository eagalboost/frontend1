...
const SubtitleAndCaption = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Subtitle and Captions`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Subtitle and Caption Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Subtitle and Caption services found.</p>
        )}
      </div>
    </section>
  );
...
export default SubtitleAndCaption;
