...
const Wix = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Wix`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Wix Development Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Wix development services found.</p>
        )}
      </div>
    </section>
  );
