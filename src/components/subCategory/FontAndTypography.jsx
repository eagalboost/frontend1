...
const FontAndTypography = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Font and Typography`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Font & Typography Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Font & Typography services found.</p>
        )}
      </div>
    </section>
  );
...
export default FontAndTypography;
