...
const MobileAppDevelopment = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Mobile App Development`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Mobile App Development Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No mobile app development services found.</p>
        )}
      </div>
    </section>
  );
