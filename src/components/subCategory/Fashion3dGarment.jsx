...
const Fashion3dGarment = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=3D Fashion Garment`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Fashion 3D Garment Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Fashion 3D Garment services found.</p>
        )}
      </div>
    </section>
  );
...
export default Fashion3dGarment;
