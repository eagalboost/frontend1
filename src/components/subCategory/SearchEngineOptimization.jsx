...
const SearchEngineOptimization = () => {
    ...
    const fetchServices = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/services?subcategory=Search Engine Optimization`
        );
  ...
  return (
    <section className="sub-category">
      <h2 className="sub-category-title">Search Engine Optimization Services</h2>
      <div className="sub-category-services-list">
        {services.length > 0 ? (
          ...
          <p>No Search Engine Optimization services found.</p>
        )}
      </div>
    </section>
  );
